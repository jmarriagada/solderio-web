...
import { ArrowRight, Newspaper, Sparkles, TrendingUp } from "lucide-react";
        ...
        <div className="relative z-20 w-full flex flex-col items-center">
          ...
          >
            ...
            <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-white/10 backdrop-blur-md border border-white/15 text-xs text-white/90 font-light mb-6 shadow-sm">
              <Newspaper className="w-3.5 h-3.5 text-[#FF8300]" />
              <span>Publicaciones Técnicas, Casos de Estudio & Noticias</span>
            </div>

            <h1 className="text-3xl sm:text-5xl md:text-6xl font-light tracking-tight text-white mb-6 leading-[1.15]">
              Blog & Centro de Noticias <br className="hidden sm:inline" />
              <span className="font-normal text-white">de Energía Solar SoldeRío</span>
            </h1>

            <p className="text-white/75 text-base md:text-xl font-light max-w-2xl mx-auto leading-relaxed">
              Casos de éxito reales en el sur, novedades de la normativa SEC, comparativas de tecnología fotovoltaica y análisis de rentabilidad para tu hogar o empresa.
            </p>
          </motion.div>
        </div>

        {/* Bottom Feature Tags */}
        <div className="relative z-20 w-full max-w-4xl mx-auto px-6 pb-8 flex flex-wrap items-center justify-center gap-3 text-xs text-white/60 font-light">
          <span>Temas Populares:</span>
          {["Casos Reales", "Ley Net Billing", "Baterías LiFePO4", "Franquicias Tributarias", "Sur de Chile"].map((tag, tIdx) => (
            <span key={tIdx} className="px-3 py-1 rounded-full bg-white/5 border border-white/10 text-white/80">
              #{tag}
            </span>
          ))}
        </div>
